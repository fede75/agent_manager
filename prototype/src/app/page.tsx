  ...
  const addEntity = (kind: string, data: any) => {
    setState((s) => {
      ...
      if (kind === "skill") {
        ...
        next.skillVersions = [{ id: `${id}:1.0.0`, ada_skill_version_id: `${id}:1.0.0`, skill_id: id, version: "1.0.0", status: "approved", change_type: "minor", breaking_change: false, release_notes: "Initial skill version", registry_record_version: "1.0.0", registry_record_revision_id: `rev-${id}`, artifact_format: "SKILL_MD", artifact_location: `s3://ada-agent-skills/${id}/1.0.0/SKILL.md`, definition_json_location: `s3://ada-agent-skills/${id}/1.0.0/definition.json`, specification_markdown: data.specification_markdown || skillMarkdown(id, "1.0.0"), instruction_summary: data.description || "Reusable agent skill", input_contract: "Structured context", output_contract: "Guided response", required_tools: [], required_mcp_servers: [], constraints: [], safety_notes: [], examples: [], compatible_runtimes: ["AgentCore Runtime"], requires_recertification: false, published_at: nowStamp(), deprecated_at: "", retirement_date: "31-12-2026" }, ...s.skillVersions];
      }
      return next;
    });
